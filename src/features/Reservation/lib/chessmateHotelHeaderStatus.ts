...
const MIRROR_HOTEL_TITLES = new Set<string>([
    ...
    // «Нора» отсюда убрана 12.09.2026: с отелем больше не работаем.
    'сан амра sun amra',
    'студио сан амра',
    ...
    'аврора inn',
    'блэк си',
    // «Лазурит» — Bnovo, отельер включил iCal-экспорт и прислал ссылки (08.08).
    'лазурит',
    ...
    'вилла оазис',
    // «Грант» — сетка отельера из OneDrive; выгрузку .xlsx кладут в папку
    // Google Drive, кнопка читает её сразу (крон каждый час).
    'грант отель',
    'грант коттеджи',
    'грант апартаменты',
...
const CHESSMATE_STATUS_BY_HOTEL_TITLE: Record<string, ChessmateHotelHeaderStatus> = {
    ...
    // «Пицунда» (TravelLine 41023, 49 номеров) — доступ в кабинет есть, занятость
    // перенесена снимком из шахматки. Для голубого нужны API-ключи (двухфакторка
    // кабинета мешает крону). Пока жёлтая (доступ есть).
    'пицунда': 'access',
    мулберри: 'active',
