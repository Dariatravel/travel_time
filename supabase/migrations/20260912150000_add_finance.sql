...
-- хранятся: они считаются из reserves + booking_cards + hotel_terms на лету
-- (src/features/Finance/lib/finance.ts), поэтому правка брони сразу меняет
-- ведомость. Долг возникает по выезду гостя.
--
...
--
-- Полный откат: DROP TABLE public.finance_adjustments, public.payouts,
-- public.hotel_payment_details, public.hotel_terms; папки src/features/Finance,
-- src/app/main/finance.
BEGIN;
...
CREATE INDEX IF NOT EXISTS payouts_hotel_idx ON public.payouts (hotel_id, paid_at DESC);
