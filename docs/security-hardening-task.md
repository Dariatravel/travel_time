...
### 7. Rollout: замена политик — строго атомарная

> ⚠️ **Старые политики нельзя оставлять «рядом» с новыми даже временно.** В PostgreSQL
> несколько PERMISSIVE-политик для одной команды объединяются через **`OR`**: пока живёт старая
> политика на `user_metadata`, повышение прав подделкой роли продолжает работать, сколько бы
> правильных новых политик мы ни добавили. Внешний смоук-тест внутри транзакции провести
> нельзя — значит «создать новые → проверить → удалить старые» оставляет **реальное окно
> уязвимости на всё время проверки**. Так делать нельзя.

Правильный порядок:

**Шаг 1 (отдельная миграция).** `user_roles` + `current_app_role()` + триггеры
(`updated_by`/`updated_at`, защита последнего админа) + **бэкофилл**.
Проверить до перехода к шагу 2: `select count(*) from public.user_roles` = 124,
распределение 110 `hotel` / 12 `operator` / 2 `admin`, и что `current_app_role()` возвращает
верную роль для тестовых пользователей. Политики пока не трогаем — система работает по-старому.

**Шаг 2 (отдельная миграция, ОДНА транзакция).** Для каждой из 12 политик в пределах одной
транзакции: `create policy` новой + `drop policy` старой. Никаких промежуточных состояний и
никаких проверок «снаружи» посередине — только атомарная замена.
DDL в PostgreSQL транзакционен: до `commit` все сессии видят старые политики, после — новые,
**окна без доступа не возникает**, релогин не нужен (роль читается из таблицы, не из JWT).
Учесть: `create/drop policy` берут блокировку на таблицу — миграция короткая, но выполнять
её лучше в период низкого трафика.

**Шаг 3 (после `commit`).** Смоук-тест ролей — отдельными сессиями трёх пользователей
(см. раздел «Смоук-тест ролей»). Если тест провалился — применить заранее подготовленную
`down`-миграцию из п.8, а не чинить на живой базе.

**Требование к готовности:** `down`-миграция шага 2 написана и **проверена на копии данных
до применения `up`** (иначе откатывать будет нечем).

**Контроль после шага 2:** ни одна политика не должна содержать `user_metadata` —
```sql
select tablename, policyname from pg_policies
where schemaname='public' and (qual like '%user_metadata%' or with_check like '%user_metadata%');
-- ожидаемо: 0 строк
```
...
**Решение (уточнено по ревью): роут — только для staff/владельца, DTO всегда полный.
Маскированный DTO не вводим.**
Обоснование фактами: единственные потребители роута — внутренняя шахматка
(`src/shared/api/hotel/hotel.ts:418` и `:911` через `yandexBackendProxy.ts:100-113`);
`src/middleware.ts` в проекте отсутствует, публичного сценария для этого эндпоинта нет.
При правилах «401 без сессии» + «403 всем, кроме staff/владельца» у маскированного ответа
не остаётся получателя — вводить его сейчас значит закладывать мёртвый код и лишнюю
поверхность атаки.
...
3. Прошедшим проверку — полный DTO (как сейчас). Персональные поля (`guest`, `phone`) видит
   только тот, кто и так имеет к ним доступ по RLS.
4. **Ключ кэша обязан включать `user_id` и вычисленный уровень доступа**, иначе ответ одного
   пользователя утечёт другому. Сейчас сегмент строится из `authorization` (`route.ts:30-33`),
   который ни на что не влияет, — заменить.
5. **Если позже понадобится публичная витрина занятости** (сайт, виджет, бот для гостей) —
   делать **отдельным эндпоинтом**: свой scope, ответ строго «занято/свободно» без
   `guest`/`phone`/`comment`/`price`, отдельный rate limit и кэш. Не расширять этот роут.
...
**Rollback:** проверки сессии/scope вводятся за feature-flag, чтобы при поломке шахматки
или подбора мгновенно вернуть прежнее поведение, не откатывая весь PR.
...
- hotel **не может**: получить непустой `list_assignable_users`; открыть calendar-роут чужого
  отеля (403); вызвать `get_raw_user_meta_data` (revoked/удалена)
- владелец отеля **может** открыть calendar-роут своего отеля и видит полный DTO
- anon: ничего из перечисленного (401 на calendar-роут)
- ни одна политика не содержит `user_metadata` (см. контроль в P0-1.7)
...
2. **Этап 2** — `list_assignable_users` (v2) + drop старой; `security_invoker`;
   сессия и scope в calendar-роуте (+ ключ кэша по user_id и уровню доступа).
